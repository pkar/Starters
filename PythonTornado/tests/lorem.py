class LoremIpsum():
  ...
  def sentence(self):
    ...
    sections = [u' '.join(random.sample(self.WORDS, random.randint(3, 12))) for i in range(random.randint(1, 5))]
    s = u', '.join(sections)
  ...
  def words(self, count=1, common=False):
    ...
    """
    if common:
      word_list = list(self.COMMON_WORDS)
    else:
      word_list = []
    c = len(word_list)
    if count > c:
      count -= c
      while count > 0:
        c = min(count, len(self.WORDS))
        count -= c
        word_list += random.sample(self.WORDS, c)
    else:
